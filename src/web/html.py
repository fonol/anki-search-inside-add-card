...
def get_reading_modal_html(note):
    ...
    if check_index():
        ...
        if note.is_pdf() and utility.misc.file_exists(source):
            editable = False
            overflow = "hidden" 
            text = get_pdf_viewer_html(note_id, source, note.get_title(), priority)
            if "/" in source:
            ...
            text = get_text_note_html(text, note_id, editable, priority)
        ...
        params = dict(note_id = note_id, title = title, source = source, time_str = time_str, img_folder = img_folder, queue_btn_text = queue_btn_text, text = text, queue_info = queue_info, queue_info_short = queue_info_short, schedule_btns=schedule_btns, queue_readings_list = queue_readings_list, overflow=overflow)
        html = html.format_map(params)
        return html
    return ""


def get_text_note_html(text, nid, editable, priority):
    """
...
def quick_sched_btn(nid, priority):
    ...
    if value == 0: 
        ...
        current_btn = f"""<div class='siac-btn siac-btn-dark-smaller' onclick='pycmd("siac-reschedule-read-next {nid} -1");'><b>Current</b></div>"""
    return f"""
...
def get_reading_modal_bottom_bar(note):
    ...
    params = dict(note_id = note_id, time_str = time_str, queue_btn_text = queue_btn_text, queue_info = queue_info, queue_info_short = queue_info_short, queue_readings_list = queue_readings_list, schedule_btns=schedule_btns )
    html = html.format_map(params)
    return html
...
def get_pdf_viewer_html(nid, source, title, priority):
    ...
    html = """
        ...
        <div id='siac-pdf-top' data-pdfpath="{pdf_path}" data-pdftitle="{pdf_title}" data-pdfid="{nid}" onwheel='pdfMouseWheel(event);' style='overflow-y: hidden;'>
            ...
            <div id="text-layer" onmousedown='pdfMouseDown(event)' onmouseup='pdfKeyup(event);' onclick='textlayerClicked(event, this);' class="textLayer"></div>
        </div>
